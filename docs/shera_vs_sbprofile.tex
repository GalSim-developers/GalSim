...
a 2d image). \sbp\ allows the choice of varying interpolation kernels for this step.
...
Note that the standard interpolation method that is used \shera\ is the aforementioned 
cubic interpolation, whereas for \sbp, it is quintic interpolation.  But the place where the
interpolations are applied differs, which could
be important.  
...
with the ideal case at the $-0.06$\% and $-0.03$\% level for Lanczos 5th and 9th order interpolation
...
the final ellipticity were $\sim 0.02$\%.
...
the ACS images, but we will almost certainly not do anything about (1).  Thus, very low-level
systematic uncertainties in what the galaxies look like are tolerable, though of course we want to
make sure that our shears that we apply are applied accurately.
...
might require 9th order Lanczos.  There might be a little room to change the order (should be $>5$),
