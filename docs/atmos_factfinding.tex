...
plane at $z=L$.  If an infinite uniform plane wave has a field
distribution $\mathcal{E}(x, y, 0)$ at $z=0$, then its Fourier
transform in the plane at $z=L$ is given by
\begin{equation}
\tilde{\mathcal{E}}(\nu_x, \nu_y, L) = \tilde{\mathcal{E}}(\nu_x,
\nu_y, 0) \times \exp{\left[-2{\rm i} \pi^2 \nu^2 / L \right]},
\end{equation}
omitting the amplitude distortions (***ADD REFS***).
...
atmosphere.  As the power spectrum $P_{\phi}(\nu)$ due to turbulence
...
that these authors use a different approach:
\begin{quote}
\emph{``The single
