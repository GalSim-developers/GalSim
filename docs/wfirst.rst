...
The galsim.wfirst module contains information and functionality that can be used to simulate
images for the WFIRST observatory.  Some of the functionality is specific to WFIRST per se, but
...
There are a number of attributes of the ``galsim.wfirst`` module, which define some numerical
parameters related to the WFIRST geometry.  Some of these parameters relate to the entire
...
pupil_plane_file
    The name of the file containing the image of the pupil plane for WFIRST,
...
jitter_rms
    The worst-case RMS jitter per axis for WFIRST in the current design (reality
...
`galsim.wfirst.allDetectorEffects`
    ...
    the various routines like `galsim.wfirst.applyNonlinearity`.
...
`galsim.wfirst.allowedPos`
    A routine to check whether WFIRST is allowed to look at a given position on a
    given date, given the constraints on orientation with respect to the sun.

`galsim.wfirst.bestPA`
    A routine to calculate the best observatory orientation angle for WFIRST when looking
...
obtained by a two-step process:
    ...
    1. Call the `galsim.wfirst.getPSF` routine and convolve the PSF with the WFIRST pixel response
...
.. autofunction:: galsim.wfirst.getBandpasses

.. autofunction:: galsim.wfirst.getSkyLevel

.. autofunction:: galsim.wfirst.getPSF

.. autofunction:: galsim.wfirst.getWCS

.. autofunction:: galsim.wfirst.findSCA

.. autofunction:: galsim.wfirst.allowedPos
...
.. autofunction:: galsim.wfirst.addReciprocityFailure

.. autofunction:: galsim.wfirst.applyIPC

.. autofunction:: galsim.wfirst.applyPersistence

.. autofunction:: galsim.wfirst.allDetectorEffects
