...
- Removed AttributeDict, which we had sitting in utilities, but which we have never used.
  (#1129)
- Documented an API change that was actually introduced in 2.3 that `_InterpolatedImage` does
...
- Added Correlated noise type as a generalization of the more specific COSMOS noise type. (#731, #1174)
- Added galaxy_sample input type with corresponding SampleGalaxy GSObject type. (#795, #1174)
- Added COSMOSValue and SampleValue value types. (#954, #1174)
- Allowed template values to be evaluated using the "$" shorthand notation. (#1138)
- Added ability to register templates by name. (#1143)
...
- Fixed error in returned variance in `CCDNoise.addNoise`, which was in e- rather than ADU.
  (#1166, #1167)
...
- Added pupil_u and pupil_v to PhotonArray and persist the values thereof in phase_psf. (#1147)
...
- Added `PupilSampler` and `PupilAnnulusSampler` photon operators. (#1176)
...
- Fixed error in ChromaticSum photon shooting when n_photons is explicitly given. (#1156, #1157)
