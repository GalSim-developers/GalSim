...
// 	$Id: fft.cpp,v 1.17 2012/01/11 14:18:51 dgru Exp $	
...
int fft::goodFFTSize(int input) {
  ...
  int Nk = static_cast<int> (ceil(exp(MIN(log2n, log2n3))-1e-5));
  return Nk;
}
...
size_t
KTable::index(int ix, int iy) const {
...
DComplex KTable::kval(int ix, int iy) const { 
  check_array();
  DComplex retval=scaleby*array[index(ix,iy)];
  if (ix<0) return conj(retval);
  else return retval;
}

void
KTable::kSet(int ix, int iy, DComplex value) {
  check_array();
...
void
KTable::accumulate(const KTable& rhs, double scalar) {
  cache.clear();	// invalidate any stored interpolations
  check_array();
...
KTable::wrap(int Nout) const {
  ...
  // Make it even:
  Nout = 2*((Nout+1)/2);
  KTable* out = new KTable(Nout, dk, DComplex(0.,0.));
  out->scaleby = scaleby;
  for (int iyin=-N/2; iyin<N/2; iyin++) {
    ...
    while (iyout >= Nout/2) iyout-=Nout;
    int ixin = 0;
    const DComplex* inptr = array + index(ixin,iyin);
    while (ixin < N/2) {
      // number of points to accumulate without conjugation:
      // Do points that do *not* need to be conjugated:
      int nx = MIN(N/2-ixin+1, Nout/2+1);
      const DComplex* inptr = array + index(ixin,iyin);
      DComplex* outptr = out->array + out->index(0,iyout);
      for (int i=0; i<nx; i++) {
	*outptr += *inptr;
	inptr++;
	outptr++;
...
XTable::wrap(int Nout) const {
  ...
  for (int iyin=-N/2; iyin<N/2; iyin++, iyout++) {
    ...
    while (ixin < N/2) {
      ...
      int nx = MIN(N/2-ixin, Nout/2-ixout);
...
DComplex 
KTable::interpolate(double kx, double ky, const Interpolant2d& interp) const {
  kx /= dk;
  ky /= dk;
...
KTable::xval(double x, double y) const { // assumes (x,y) in physical units
  ...
  if (abs(x) > PI || abs(y) > PI) throw FFTOutofRange(" (x,y) too big in xval()");
  DComplex I(0.,1.);
...
size_t
XTable::index(int ix, int iy) const {
...
void
XTable::xSet(int ix, int iy, double value) {
  check_array();
  cache.clear();	// invalidate any stored interpolations
  array[index(ix,iy)]=value/scaleby;
  return;
...
void
XTable::accumulate(const XTable& rhs, double scalar) {
  check_array();
  cache.clear();	// invalidate any stored interpolations
  scalar *= rhs.scaleby / scaleby;
...
double
XTable::interpolate(double x, double y, const Interpolant2d& interp) const {
...
XTable::kval(double kx, double ky) const {
  ...
  kx*=dx; ky*=dx;
  if (abs(kx) > PI || abs(ky) > PI) throw FFTOutofRange("XTable::kval() args out of range");
  DComplex I(0.,1.);
