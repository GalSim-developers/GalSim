// $Id: Interpolant.cpp,v 1.4 2012/01/06 19:54:55 garyb Exp $
...
double
Interpolant::xvalWrapped(double x, int N) const {
...
double 
Lanczos::uCalc(double u) const {
  double vp=n*(2*u+1);
  double vm=n*(2*u-1);
  double retval = (vm-1.)*Si(PI*(vm-1.))
    -(vm+1.)*Si(PI*(vm+1.))
    -(vp-1.)*Si(PI*(vp-1.))
    +(vp+1.)*Si(PI*(vp+1.));
  return retval/(2.*PI);
...
public:
  ...
  double operator()(double x) const {return c.xval(x)*cos(2*PI*u*x);}
private:
  double u;
  const Cubic& c;
};
