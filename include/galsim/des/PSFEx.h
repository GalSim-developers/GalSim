...
// Header files from Peter's ShapeLens package.
...
public:
    PSFEx(std::string filename) : polzero(2), polscale(2), psfaxis(3) 
    {
        ...
        std::cout << "# " << psfaxis[0] << "\t" << psfaxis[1] << "\t" << poldeg << " -> " 
            << psfaxis[2] << std::endl;
        ...
        for (int k = 0; k < psfaxis[2]; k++)
            fits_read_col(fptr, FITS::getDataType(data_t(0)), 1, 1, 1 + k*psfaxis[0]*psfaxis[1],
                          psfaxis[0]*psfaxis[1], NULL, basis[k].ptr(), &anynull, &status);
    ...
    data_t pixel_lanczos3(data_t relx, data_t rely, data_t centerx, data_t centery)  
    {
        ...
        if (fabs(relxp) > (psfaxis[0]-1)/2.-INTERPFAC || 
            fabs(relyp) > (psfaxis[1]-1)/2.-INTERPFAC) {
            std::cerr << "interpolating out of bounds at (" << relx << "," << rely << ")." 
                << std::endl;
            return 0;
        }
    ...
    double pixel_sampled_interpolated(int i, int j, double x, double y) 
    {
        ...
        std::cerr << "warning: requesting pixel outside valid range " << i << " " << j 
            << std::endl;
        return 0;
    }
    ...
    void fillObject(Object& psf) 
    {
        ...
        for (int i=0; i < psf.size(); i++) {
            ...
            if (fabs(P(0)-psf.centroid(0)) < m && fabs(P(1) - psf.centroid(1)) < m) {
                psf(i) = pixel_lanczos3(P(0) - psf.centroid(0), 
                                        P(1) - psf.centroid(1), 
                                        psf.centroid(0), 
                                        psf.centroid(1));
                sum += psf(i);
...
private:
    ...
    // normalized sinc function, see PSFEx manual
