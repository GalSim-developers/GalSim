...
def show_shear(E,B,title):
    ...
    my_ps = galsim.PowerSpectrum(E,B, units=galsim.radians)
    # Get the shears and kappa from the PS
    print "Drawing shears and convergences on a grid"
    g1, g2, kappa = my_ps.buildGriddedShears(grid_spacing = 0.02, ngrid = 120, units=galsim.degrees,
    ...
    pylab.colorbar()
    magnify = 50.
    pylab.quiver(magnify * gx, magnify * gy, scale=50, headwidth=0, pivot='middle')
    pylab.title(title)
    pylab.figure()
    ...
    print np.var(ke), np.var(kappa)

tab_ps = galsim.LookupTable(file='./data/cosmo-fid.zmed1.00.out', interpolant='linear')
show_shear(tab_ps, None, "E-mode")
