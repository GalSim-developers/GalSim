...
smallim_size = 128 # size of image when we test correlation function properties using small inputs
...
# number of CorrFuncs to average over to get good statistics for noise generation test
nsum_test = 10
...
def test_output_generation_basic():
    ...
    ncf.draw(refim, dx=1.)
    # Generate a large image containing noise according to this function, and 
...
def test_output_generation_rotated():
    ...
    ncf = galsim.correlatednoise.CorrFunc(ynoise_large, dx=1.)
    print ncf.xValue(galsim.PositionD(0., 0.))
    # Then loop over some angles
    angles = [18.7 * galsim.degrees, 135. * galsim.degrees, 340.2 * galsim.degrees]
    for angle in angles:
        ...
        outimage = galsim.ImageD(1024, 1024)#xnoise_large.bounds)
        ...
        ncf_2ndlevel.draw(testim, dx=1.)
        import matplotlib.pyplot as plt
        plt.pcolor(refim.array, vmin=-.1, vmax=1.1); plt.colorbar()
        plt.figure()
        #plt.pcolor(testim.array - refim.array, vmin=-.1, vmax=.1); plt.colorbar()
        #plt.figure()
        plt.pcolor(testim.array, vmin=-.1, vmax=1.1); plt.colorbar()
        print np.std(testim.array - refim.array)
        print ncf_rot.xValue(galsim.PositionD(0., 0.)),ncf_2ndlevel.xValue(galsim.PositionD(0., 0.))
        plt.show()
        np.testing.assert_array_almost_equal(
...
def test_output_generation_magnified():
    ...
    """
    # Get the noise correlation function from a small image
    ncf = galsim.correlatednoise.CorrFunc(ynoise_small, dx=1.)
    scales = [1.3, 2.1] # these have to be half sensible
    for scale in scales:
        ncf_scl = ncf.createMagnified(scale)
        # Generate a large image containing noise according to this function
        outimage = galsim.ImageD(xnoise_large.bounds)
        ncf_scl.applyNoiseTo(outimage, dx=1., dev=glob_ud)
        # Then generate a 2nd level CorrFunc from this noise field: should be approximately
        # equal to the input
        ncf_2ndlevel = galsim.correlatednoise.CorrFunc(outimage, dx=1.)
        ...
        #plt.show()
        np.testing.assert_array_almost_equal(
            outim1.array, outim2.array, decimal=1, # only matches to 1dp, and variance always seems 
                                                   # *larger* in the 2nd level correlation function,
                                                   # is there perhaps a theorem about this?
