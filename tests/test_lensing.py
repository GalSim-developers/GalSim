...
def test_power_spectrum_kappa():
    ...
    # in order to get this to match the independently tested kappa inversion.  And the differences
    # between the kappaKaiserSquires and the buildGriddedShears kappas look a bit artifact-y: they
    # do not agree perfectly as you might expect.  Not sure!
    rseed=177774
    ngrid=100
    dx_grid_arcmin = .9
    # First lookup a cosmologically relevant power spectrum
    tab_ps = galsim.LookupTable(
        file='../examples/data/cosmo-fid.zmed1.00.out', interpolant='linear')
    ...
    np.testing.assert_array_almost_equal(
        ...
        err_msg="E-mode only PowerSpectrum output kappaE does not match KS inversion to 3d.p.")
    # Test that B-mode kappa matches zero to some sensible accuracy
    np.testing.assert_array_almost_equal(
        kB_ks, np.zeros_like(kE_ks), decimal=3,
        err_msg="E-mode only PowerSpectrum output kappaB from KS does not match zero to 3d.p.")
    ...
    # Test that kappa matches zero to some sensible accuracy
    np.testing.assert_array_almost_equal(
        k_test, np.zeros_like(k_test), decimal=3,
        err_msg="B-mode only PowerSpectrum output kappa does not match zero to 3d.p.")
    # Test that E-mode kappa matches zero to some sensible accuracy
    np.testing.assert_array_almost_equal(
        kE_ks, np.zeros_like(kB_ks), decimal=3,
