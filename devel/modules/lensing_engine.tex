...
is $2\pi/L$, so we can think of this quantity as the grid spacing in Fourier space.
...
P(k_x, k_y).
\eeq
Note that this equation assumes that our grid is infinite.  With a limited $k$
range, we can rewrite Eq.~\ref{E:alt-shearvar} as
...
\kmax, Eqs.~\ref{E:shearvar}, \ref{E:alt-shearvar}, and
...
interpretation.  In the next section, we present some of the standard results of
Fourier theory using the angular frequency, non-unitary convention above, since we will require them in Section \ref{sect:DFTPS}.
...
e^{-\sigma^2 k^2/2} \,\rmd k\right]^2 \\
 &= \frac{P_0}{2\pi\sigma^2}\left[ \mathrm{Erf}(\kmax\sigma/\sqrt{2}) - \mathrm{Erf}(\kmin\sigma/\sqrt{2})\right]^2.\label{E:vargauss}
\end{align}
...
$\mathrm{Erf}(\kmin\sigma/\sqrt{2})=0.04$, $0.08$, and $0.04$.  With
$P_0=1$ arcsec$^2$, the predicted variances for the three grids are
$1.79\times 10^{-6}$, $1.64\times 10^{-6}$, and $4.47\times 10^{-7}$.
...
the results are 4, 12, and 4\% larger than the predictions.
Alternatively, if we consider the prediction from
Eq.~\ref{E:vargauss}, our results are consistently 4\% below what we
would expect if $\kmin=0$; the significance of this last comparison is
unclear, but it seems suspiciously similar to the final results in \S\ref{S:testpk}.
...
a modified version of it: We use $P(k)=P_0$ for $k<$\klim, where
...
$2.36\times 10^{-7}$.  These are all $\sim 5.5$\% below my naive calculation.
...
the number of realizations.} to $2.40\times 10^{-7}$, $9.60\times 10^{-7}$, and
$2.40\times 10^{-7}$, so they are now only 4\% lower than the
predictions.  This result supports the claim that finite-gridding
...
finite $\ell$-binning on the theoretical power spectrum, unless they
