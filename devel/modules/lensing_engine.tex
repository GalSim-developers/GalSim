...
P(k_x, k_y).
\eeq
\textbf{More comments from Rachel: Actually on further reflection I
  think that there is something to be careful about here.  You could
  imagine that the two integrals might give the same thing if
  $\kmin\ll\kmax$ and if the power at large $k$ is negligible, but
  actually, Eq.~\ref{E:shearvar} is integrating effectively over all
  four quadrants in the $k_x$ vs. $k_y$ plane, whereas
  Eq.~\ref{E:alt-shearvar} integrates over only one of them.  So they
  should give a factor of four difference in the results.}
...
are dimensions of angle$^2$.  By Eq.~\ref{E:shearvar} calculations, this means that
\begin{align}
...
\textbf{Note from Rachel: That assumes we can integrate in a circle in
  $k$-space.  Okay, really it's an annulus but $\kmin\ll\kmax$ so I'm
  calling it a circle.  Anyway, if we use the Eq.~\ref{E:alt-shearvar}
  for integration within a square, I
  get something that is about a factor of 4 lower. My first gut
  reaction is that this must be wrong, but then I noticed the issue
  raised in the previous section on the same commit as this. }
...
are all okay.  \textbf{But now I'm a tad confused why it's not
  consistent with the variance calculation on a square grid. However,
  that calculation on the square grid seems to ignore 3 quadrants in
  $k$-space, so perhaps that's the issue, and everything is fine here?
  i.e., perhaps those predictions need to by multiplied by 4 anyway?
  -RM}
...
let's check the results anyway.  If we say $P(k)=P_0$, and use Eq.~\ref{E:alt-shearvar} then we expect
\beq
\mathrm{Var}(\gamma) = \frac{P_0}{(2\pi)^2} \times (\kmax-\kmin)^2
\sim \frac{P_0 \kmax^2}{(2\pi)^2}.
\eeq

If we use Eq.~\ref{E:shearvar}, then we expect
\beq
\mathrm{Var}(\gamma) = \frac{P_0}{4\pi}\times (\kmax-\kmin)^2 \sim \frac{P_0\kmax^2}{4\pi}
\eeq
which is higher by a factor of $\pi$.
...
what I find.  So the variance test seems to work here, giving a result
consistent with Eq.~\ref{E:alt-shearvar} (but note that the previous
test gave results consistent with those from Eq.~\ref{E:shearvar}, hmmm).  \textbf{Note
