...
def PlotStatsForParam(config,filename_config,param_name):
    """
    ...
    for iv,value in enumerate(param['values']):
        ...
        filename_out = 'results.%s.%s.%03d.cat' % (filename_config,param_name,iv)
    ...
    pylab.xscale('log')
    pylab.errorbar(param['values'],moments_E1_diff_mean,yerr=moments_E1_diff_stdm,fmt='b+-')
    pylab.errorbar(param['values'],moments_E2_diff_mean,yerr=moments_E2_diff_stdm,fmt='rx-')
    pylab.plot(param['values'],moments_E1_diff_medn,'b+--')
    pylab.plot(param['values'],moments_E2_diff_medn,'rx--')
    pylab.ylabel('Ei_fft - Ei_photon')
    pylab.xlabel(param_name)
    pylab.xlim([min(param['values'])*0.5, max(param['values'])*1.5])
    pylab.legend(['mean diff e1','mean diff e2','median diff e1','median diff e2'])
    filename_fig = 'fig.moments.%s.%s.pdf' % (filename_config,param_name)
    pylab.savefig(filename_fig)
    # pylab.show()
    pylab.close()
    logger.info('saved figure %s' % filename_fig)
    ...
    pylab.xscale('log')
    pylab.errorbar(param['values'],hsmcorr_E1_diff_mean,yerr=hsmcorr_E1_diff_stdm,fmt='b+-')
    pylab.errorbar(param['values'],hsmcorr_E2_diff_mean,yerr=hsmcorr_E2_diff_stdm,fmt='rx-')
    pylab.plot(param['values'],hsmcorr_E1_diff_medn,'b+--')
    pylab.plot(param['values'],hsmcorr_E2_diff_medn,'rx--')
    pylab.ylabel('Ei_fft - Ei_photon')
    pylab.xlabel(param_name)
    pylab.xlim([min(param['values'])*0.5, max(param['values'])*1.5])
    pylab.legend(['mean diff e1','mean diff e2','median diff e1','median diff e2'])
    filename_fig = 'fig.hsmcorr.%s.%s.pdf' % (filename_config,param_name)
    pylab.savefig(filename_fig)
    # pylab.show()
    pylab.close()
    logger.info('saved figure %s' % filename_fig)
    ...
    pylab.xscale('log')
    pylab.errorbar(param['values'],moments_sigma_diff_mean,yerr=moments_sigma_diff_stdm,fmt='b+-')
    pylab.errorbar(param['values'],hsmcorr_sigma_diff_mean,yerr=hsmcorr_sigma_diff_stdm,fmt='b+-')
    pylab.plot(param['values'],moments_sigma_diff_medn,'rx--')
    pylab.plot(param['values'],hsmcorr_sigma_diff_medn,'b+--')
    pylab.ylabel('sigma_fft - sigma_photon')
    pylab.xlabel(param_name)
    pylab.xlim([min(param['values'])*0.5, max(param['values'])*1.5])
    pylab.legend(['mean diff sigma moments','mean diff sigma hsmcorr','median diff sigma moments','median diff sigma hsmcorr'])
    filename_fig = 'fig.sigma.%s.%s.pdf' % (filename_config,param_name)
    pylab.savefig(filename_fig)
    # pylab.show()
    pylab.close()
    logger.info('saved figure %s' % filename_fig)
...
if __name__ == "__main__":
    ...
    for param_name in config['vary_params'].keys():
        PlotStatsForParam(config,filename_config,param_name)
