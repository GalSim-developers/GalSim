...
int main(int argc, char **argv) {
   ...
   RECT_IMAGE AtlasImage, PSFImage;
   int status;
   long i,j;
   ...
   if (argc!=8) {
     fprintf(stderr,"Usage:\n\t%s gal_image_file PSF_image_file gal_x_centroid_guess gal_y_centroid_guess skyvar shear_estimator sky\n",argv[0]);
      exit(1);
   }
   ...
   status = (int) general_shear_estimator(&AtlasImage, &PSFImage, &GalaxyData, &PSFData, argv[6], 0xe);

   /* Compute very rough error estimate */
   shearsig = sqrt( 4. * Pi * skyvar ) * GalaxyData.sigma / (GalaxyData.resolution * GalaxyData.flux);

   printf("%d %13.9lf %13.9lf %c %13.9lf %13.9f %13.9f %13.9f %13.9f\n", status, GalaxyData.e1, GalaxyData.e2,GalaxyData.meas_type, GalaxyData.responsivity, GalaxyData.resolution, shearsig,GalaxyData.sigma,GalaxyData.flux);
