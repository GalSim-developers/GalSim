...
void qho1d_wf_1(long nx, double xmin, double xstep, long Nmax,
   double sigma, double **psi) {
...
void find_mom_1(RECT_IMAGE *data, double **moments, int max_order,
...
void find_mom_2(RECT_IMAGE *data, double **moments, int max_order,
...
void find_ellipmom_1(RECT_IMAGE *data, double x0, double y0, double Mxx,
   ...
   *A = *Bx = *By = *Cxx = *Cxy = *Cyy = *rho4w = 0.;
   for(x=xmin;x<=xmax;x++) {
      maskptr  = data->mask[x] + ymin;    /* Use these pointers to speed up referencing arrays */
...
void find_ellipmom_2(RECT_IMAGE *data, double *A, double *x0, double *y0,
   double *Mxx, double *Mxy, double *Myy, double *rho4, double epsilon,
   int *num_iter) {
...
void fast_convolve_image_1(RECT_IMAGE *image1, RECT_IMAGE *image2,
   RECT_IMAGE *image_out) {
   ...
   /* Build the arrays for FFT -- put m1 and m2 into the real and imaginary parts of Bx, respectively. */
...
/* build_index_1
 * *** BUILDS AN INDEX MATRIX ***
 *
 * The index matrix sindex[m][n] yields the index of the m:n shapelet
 * basis function.  Here 0<=m+n<=max_order, and the index values range
 * from 0 through ((max_order+1)*(max_order+2))/2-1.
 *
 * Arguments:
 *   max_order: maximum order of shapelets
 * > num_shapelets: number of shapelet basis functions used
 *
 * Returns an index matrix (in imatrix format, to be freed later with
 * free_imatrix or a similar routine -- range [0..max_order][0..max_order],
 * although strictly speaking almost half of the memory is unused)
 */

int** build_index_1(int max_order, int *num_shapelets) {

   int m, n, i;
   int **sindex;

   sindex = imatrix(0,max_order,0,max_order);
   i=0;
   for(m=0;m<=max_order;m++)
      for(n=0;n<=max_order-m;n++)
         sindex[m][n] = i++;
   *num_shapelets = i;

   /* Note that we do not de-allocate sindex here, it is returned and must be
    * freed later.
    */
   return sindex;
}

/* convolution_matrix_1
 * *** COMPUTES THE PSF CONVOLUTION MATRIX IN SHAPELETS BASIS ***
 *
 * Computes the matrix P in: (f*g)_{nx,ny} = sum P_{nx,ny}{mx,my} f_{mx,my}
 * for given PSF g.
 *
 * Arguments:
 *   psf_moments: psf_moment[lx][ly] is the (lx,ly) moment of the PSF
 *   max_order_psf: maximum order lx+ly considered for the PSF
 *   max_order_gal: maximum order mx+my considered for the galaxy
 *   sigma_f: width for measuring the intrinsic galaxy image
 *   sigma_g: width for measuring the PSF
 *   sigma_h: width for measuring the post-convolution galaxy image
 * > psfmatrix: psfmatrix[index[nx][ny]][index[mx][my]] is the convolution matrix.
 *   sindex: index table for psfmatrix indices
 */

void convolution_matrix_1(double **psf_moments, int max_order_psf, int max_order_gal,
   double sigma_f, double sigma_g, double sigma_h, double **psfmatrix, int **sindex) {

   int lx,ly,mx,my,nx,ny;
   long l,m,n, max_order_tot;
   double ***Cnml;
   double a,b,c,nuinv;
   double ab, bc, ac, a21, b21, c21;
   double factor1;
   double *factor2;
   double *psfmatrix_element;

   max_order_tot = (max_order_psf>max_order_gal)? max_order_psf: max_order_gal;
   factor2 = dvector(0,max_order_tot);

   /* Allocate memory for the C-tensor of Refregier & Bacon astro-ph/0105179. */
   Cnml = (double ***) malloc ((size_t)( (max_order_tot+1) * sizeof(double**) ));
   for(n=0;n<=max_order_tot;n++) {
      Cnml[n] = dmatrix(0,max_order_tot,0,max_order_tot);
      for(m=0;m<=max_order_tot;m++)
         for(l=0;l<=max_order_tot;l++)
            Cnml[n][m][l] = 0.;
   }

   /* Now let's compute Refregier & Bacon's parameters a, b, and c.  These satisfy:
    * a^2 + b^2 + c^2 = 2.
    */
   nuinv = sqrt( sigma_f * sigma_f + sigma_g * sigma_g + sigma_h * sigma_h );
   a = sqrt(2.) * sigma_g / nuinv;
   b = sqrt(2.) * sigma_f / nuinv;
   c = sqrt(2.) * sigma_h / nuinv;
   ab = a*b;
   ac = a*c;
   bc = b*c;
   a21 = a*a-1.;
   ...
   /* Initialize the recursion of L[n][m][l] at l=0.  We will put the L's in the
    * memory slots for the C's and then do appropriate multiplication at the end.
    */
   Cnml[0][0][0] = 1.;
   for(n=1;n<max_order_tot;n+=2) Cnml[n+1][0][0] = 2 * n * c21 * Cnml[n-1][0][0];
   for(m=1;m<max_order_tot;m+=2) Cnml[0][m+1][0] = 2 * m * b21 * Cnml[0][m-1][0];
   for(n=1;n<=max_order_tot;n++) {
      Cnml[n][1][0] = 2 * n * bc * Cnml[n-1][0][0];
      for(m=1;m<max_order_tot;m++)
         Cnml[n][m+1][0] = 2 * ( n*bc*Cnml[n-1][m][0] + m*b21*Cnml[n][m-1][0] );
      }

   /* Now let's work the recursion relation forward to l>0. */
   for(l=0;l<max_order_tot;l++)
      for(m=0;m<=max_order_tot;m++)
         for(n=0;n<=max_order_tot;n++)
            if ((l+m+n)%2) {
               Cnml[n][m][l+1] = 0.;
               if (n>0) Cnml[n][m][l+1] += 2 * n * ac * Cnml[n-1][m][l];
               if (m>0) Cnml[n][m][l+1] += 2 * m * ab * Cnml[n][m-1][l];
               if (l>0) Cnml[n][m][l+1] += 2 * l * a21* Cnml[n][m][l-1];
            }

   /* Convert from L coefs to C coefs (Refregier & Bacon Eq.6) */
   factor1 = 2.662670727601 / nuinv * sqrt( sigma_f*sigma_g*sigma_h );
   factor2[0] = 1;
   for(l=1;l<=max_order_tot;l++)
      factor2[l] = factor2[l-1] / sqrt(2.*l);
   for(l=0;l<=max_order_tot;l++)
      for(m=0;m<=max_order_tot;m++)
         for(n=0;n<=max_order_tot;n++) {
            Cnml[n][m][l] *= factor1 * factor2[n] * factor2[m] * factor2[l];
            if ((l+m+3*n)%4==2) Cnml[n][m][l] *= -1.;
         }

#ifdef TEST_SHAPELETS_001
   printf("sigmas: f=%lg, g=%lg, h=%lg\n", sigma_f, sigma_g, sigma_h);
   ...
   /* Now for the PSF moments */
   for(nx=0;nx<=max_order_gal;nx++)
      for(ny=0;ny<=max_order_gal-nx;ny++)
         for(mx=0;mx<=max_order_gal;mx++)
            for(my=0;my<=max_order_gal-mx;my++) {

               /* Set pointer to matrix element; initialize */
               psfmatrix_element = psfmatrix[sindex[nx][ny]] + sindex[mx][my];
               *psfmatrix_element = 0.;

               /* Add up the matrix elements */
               for(lx=0;lx<=max_order_psf;lx++)
                  for(ly=0;ly<=max_order_psf-lx;ly++)
                     *psfmatrix_element += psf_moments[lx][ly] * Cnml[nx][mx][lx]
                                                               * Cnml[ny][my][ly];
            }

   /* Clean up memory */
   for(n=0;n<=max_order_tot;n++) free_dmatrix(Cnml[n],0,max_order_tot,0,max_order_tot);
   free((char*)Cnml);
   free_dvector(factor2,0,max_order_tot);
}
 ...
 * Takes a matrix:  [ a  b ]
 *                  [ c  d ]
 * and computes the inverse.  The result is stored to the original locations
...
void shearmult(double e1a, double e2a, double e1b, double e2b, double *e1out, double *e2out) {
...
void psf_corr_bj(double Tratio, double e1p, double e2p, double a4p, double e1o,
...
void psf_corr_linear(double Tratio, double e1p, double e2p, double a4p, double e1o,
   double e2o, double a4o, double *e1, double *e2) {
   ...
   deltamu *= 0.5;
   deltaeta *= -1.0;
   R = ( 1 - 2*deltamu - deltaeta*EI - sig2ratio/coshetao ) / ( -deltaeta/EI + 1-2*deltamu ) ;
...
unsigned int psf_corr_ksb_1(RECT_IMAGE *gal_image, RECT_IMAGE *PSF, double *e1, double *e2,
...
#define FAILED_MOMENTS (-1000.0)
   ...
   *flux_gal = 3.544907701811 * *sig_gal * moments[0][0];
   ...
   y0 = *y0_psf;
   sigma0 = *sig_psf;
   find_mom_2(PSF, psfmoms, KSB_MOMENTS_MAX, x0_psf, y0_psf, sig_psf,
      1.0e-6, &num_iter);
   if (num_iter == NUM_ITER_DEFAULT) {
   ...
   *R = 1. - (*sig_psf**sig_psf)/(*sig_gal**sig_gal);
   ...
   PQQ = opgQQ - opsQQ*ppgQQ*ppsQQ - opsQQ*ppgQU*ppsUQ - opsQU*ppgUQ*ppsQQ - opsQU*ppgUU*ppsUQ;
   PQU = opgQU - opsQQ*ppgQQ*ppsQU - opsQQ*ppgQU*ppsUU - opsQU*ppgUQ*ppsQU - opsQU*ppgUU*ppsUU;
   PUQ = opgUQ - opsUQ*ppgQQ*ppsQQ - opsUQ*ppgQU*ppsUQ - opsUU*ppgUQ*ppsQQ - opsUU*ppgUU*ppsUQ;
   PUU = opgUU - opsUQ*ppgQQ*ppsQU - opsUQ*ppgQU*ppsUU - opsUU*ppgUQ*ppsQU - opsUU*ppgUU*ppsUU;
...
unsigned int psf_corr_regauss(RECT_IMAGE *gal_image, RECT_IMAGE *PSF, double *e1,
   ...
   double dx, dy, a2, b2, two_phi;
   double x0_old, y0_old, Mfxx, Mfxy, Mfyy, detMf, Minvf_xx, Minvf_xy, Minvf_yy;
   ...
   *e1 = *e2 = *R = FAILED_MOMENTS;
   ...
   if (flags & 0x00000004) {
      if (fgauss_xmin < fgauss_xctr - NSIG_RG*fgauss_xsig) fgauss_xmin = (long) floor(fgauss_xctr - NSIG_RG*fgauss_xsig);
      if (fgauss_xmax > fgauss_xctr + NSIG_RG*fgauss_xsig) fgauss_xmax = (long) ceil (fgauss_xctr + NSIG_RG*fgauss_xsig);
      if (fgauss_ymin < fgauss_yctr - NSIG_RG*fgauss_ysig) fgauss_ymin = (long) floor(fgauss_yctr - NSIG_RG*fgauss_ysig);
      if (fgauss_ymax > fgauss_yctr + NSIG_RG*fgauss_ysig) fgauss_ymax = (long) ceil (fgauss_yctr + NSIG_RG*fgauss_ysig);
   }
   ...
   for(x=fgauss.xmin;x<=fgauss.xmax;x++)
      for(y=fgauss.ymin;y<=fgauss.ymax;y++) {
         dx = x - *x0_gal + *x0_psf;
         dy = y - *y0_gal + *x0_psf;
         sum += fgauss.image[x][y] = exp (-0.5 * ( Minvf_xx*dx*dx + Minvf_yy*dy*dy )
                                          - Minvf_xy*dx*dy);
      }
   for(x=fgauss.xmin;x<=fgauss.xmax;x++)
      for(y=fgauss.ymin;y<=fgauss.ymax;y++)
         fgauss.image[x][y] *= (*flux_gal)/(sum*flux_psf); /* Properly normalize fgauss */

   /* Figure out the size of the bounding box for the PSF residual.  We don't necessarily need the whole PSF,
    * just the part that will affect regions inside the NSIG_RG2 sigma ellipse of the Intensity.
    */
   pxmin = PSF->xmin;
   pxmax = PSF->xmax;
   pymin = PSF->ymin;
   ...
   allocate_rect_image(&Iprime, gal_image->xmin,gal_image->xmax,
                                gal_image->ymin,gal_image->ymax);
   for(x=gal_image->xmin;x<=gal_image->xmax;x++)
      for(y=gal_image->ymin;y<=gal_image->ymax;y++)
   ...
   *sig_gal = pow( Mxxgal*Myygal - Mxygal*Mxygal, 0.25);
   ...
   e2psf = 2 * Mxypsf / Tpsf;
   psf_corr_bj(Tpsf/Tgal, e1psf, e2psf, 0., e1gal, e2gal, 0.5*rho4gal-1.,
      e1, e2); /* Use 0 for radial 4th moment of PSF because it's been
                * re-Gaussianized.
                */
   *R = 1. - Tpsf/Tgal;
...
/* measure_shape_ext
 * *** MEASURES THE SHAPE OF A GALAXY, INCLUDING PSF CORRECTION ***
 *
 * Computes the apparent ellipticity (e1,e2) of a galaxy.
 * An effective resolution factor R is also returned.
 *
 * Flag options:
 *   0x00000001: use sigma^2 for deconvolved image that is difference of sigma^2
 *      for galaxy and PSF (default is sigma^2_deconv = sigma^2_gal)
 ...
 *   0x00000010: use input scale radius for galaxy (also uses input centroid if
 *      flag 0x00000002 is not set)
 *
 * Arguments:
 *   gal_image: image of the galaxy as measured (i.e. not deconvolved)
 *   PSF: image of point spread function
 *   max_order_psf: shapelet order to which PSF is modeled
 *   max_order_gal: shapelet order to which galaxy is modeled
 * > e1: + ellipticity
 * > e2: x ellipticity
 * > responsivity: shear responsivity d<e>/dgamma
 * > R: effective resolution factor (0 = unresolved, 1 = well resolved)
 *   flags: controls options for shear measurement
 * > x0_gal: guess for galaxy centroid (x) [replaced with best value]
 * > y0_gal: guess for galaxy centroid (y) [replaced with best value]
 * > sig_gal: guess for galaxy sigma [replaced with best value]
 * > flux_gal: galaxy circular flux
 * > x0_psf: guess for PSF centroid (x) [replaced with best value]
 * > y0_psf: guess for PSF centroid (y) [replaced with best value]
 * > sig_psf: guess for PSF sigma [replaced with best value]
 *
...
unsigned int measure_shape_ext(RECT_IMAGE *gal_image, RECT_IMAGE *PSF, int max_order_psf,
   int max_order_gal, double *e1, double *e2, double *responsivity,
   double *R, unsigned long flags, double *x0_gal, double *y0_gal, double *sig_gal,
   double *flux_gal, double *x0_psf, double *y0_psf, double *sig_psf) {
   ...
   int **index_matrix;
   double **moments_psf, **moments_gal, **moments_gal_deconv, **psfmatrix;
   double sig_gal_deconv;
   double x0_gal_orig, y0_gal_orig, sig_gal_orig;
   double x0_psf_orig, y0_psf_orig, sig_psf_orig;
   int nx,ny,mx,my;
   int num_iter_centroid = 0;

   /* Initialize -- if we don't set the outputs, they will be reported
    * as failures.
    */
   *e1 = *e2 = *responsivity = FAILED_MOMENTS;

   /* Save original quantities */
   x0_gal_orig  = *x0_gal;
   y0_gal_orig  = *y0_gal;
   sig_gal_orig = *sig_gal;
   x0_psf_orig  = *x0_gal;
   y0_psf_orig  = *y0_gal;
   sig_psf_orig = *sig_gal;

   /* Measure the PSF */
   moments_psf = dmatrix(0,max_order_psf,0,max_order_psf);
   find_mom_2(PSF, moments_psf, max_order_psf, x0_psf, y0_psf, sig_psf,
      1.0e-6, &num_iter);
   if (num_iter==NUM_ITER_DEFAULT) {
      status |= 0x1;
      find_mom_1(PSF, moments_psf, max_order_psf, *x0_psf=x0_psf_orig,
         *y0_psf=y0_psf_orig, *sig_psf=sig_psf_orig);

      /* Note that this restores x0, y0, sig to their original values */
   }

   /* Measure the galaxy */
   moments_gal = dmatrix(0,max_order_gal,0,max_order_gal);
   find_mom_2(gal_image, moments_gal, max_order_gal, x0_gal, y0_gal,
      sig_gal, 1.0e-6, &num_iter);
   if (num_iter==NUM_ITER_DEFAULT || (flags&0x00000010)) {
      status |= 0x2;
      find_mom_1(gal_image, moments_gal, max_order_gal,
         *x0_gal=x0_gal_orig, *y0_gal=y0_gal_orig,
         *sig_gal=sig_gal_orig);
   }
   *flux_gal = 3.544907701811 * *sig_gal * moments_gal[0][0];

   /* Compute resolution factor */
   *R = 1. - (*sig_psf * *sig_psf) / (*sig_gal * *sig_gal);

   /* Select radius of the galaxy's Gaussian profile */
   if (flags & 0x00000004) *sig_gal *= 1.414213562373;
   if (flags & 0x00000008) *sig_gal *= 2.0;

   /* Select deconvolution radius */
   sig_gal_deconv = (flags & 0x00000001)?
                    sqrt(*sig_gal * *sig_gal - *sig_psf * *sig_psf)
                    : *sig_gal;

   /* Build convolution matrix */
   index_matrix = build_index_1(max_order_gal, &num_shapelets_gal);
...
#ifdef TEST_SHAPELETS_002
   printf("PSF deconvolution matrix:\n");
   for(mx=0;mx<=2;mx++)
      for(my=0;mx+my<=2;my++)
         for(nx=0;nx<=max_order_gal;nx++)
            for(ny=0;nx+ny<=max_order_gal;ny++)
               printf("[%1d,%1d]<-[%1d,%1d]:  %9.6lf\n", mx,my,nx,ny,
                  psfmatrix[index_matrix[mx][my]][index_matrix[nx][ny]]);
#endif

   /* PSF deconvolution and centroiding */
   x0_gal_orig = *x0_gal;
   y0_gal_orig = *y0_gal;

   while (resid_error > 1.0e-6) {

      /* Get PSF-deconvolved moments */
      moments_gal_deconv = dmatrix(0,max_order_gal,0,max_order_gal);
      for(mx=0;mx<=max_order_gal;mx++)
         for(my=0;mx+my<=max_order_gal;my++) {
            moments_gal_deconv[mx][my] = 0.;
            for(nx=0;nx<=max_order_gal;nx++)
               for(ny=0;nx+ny<=max_order_gal;ny++)
                  moments_gal_deconv[mx][my] += moments_gal[nx][ny]
                     * psfmatrix[index_matrix[mx][my]][index_matrix[nx][ny]];
            }

      if (flags & 0x00000002) { /* if we are iterating to find the centroid */

         /* Get translation distance */
         dx     = moments_gal_deconv[1][0] / (0.7071067811865*moments_gal_deconv[0][0] - moments_gal_deconv[2][0]);
         dy     = moments_gal_deconv[0][1] / (0.7071067811865*moments_gal_deconv[0][0] - moments_gal_deconv[0][2]);
         if (dx >  BOUND_CORRECT_WEIGHT) dx =  BOUND_CORRECT_WEIGHT;
         if (dx < -BOUND_CORRECT_WEIGHT) dx = -BOUND_CORRECT_WEIGHT;
         if (dy >  BOUND_CORRECT_WEIGHT) dy =  BOUND_CORRECT_WEIGHT;
         if (dy < -BOUND_CORRECT_WEIGHT) dy = -BOUND_CORRECT_WEIGHT;
         resid_error = (fabs(dx)>fabs(dy))? fabs(dx): fabs(dy);
         *x0_gal += dx * sig_gal_deconv;
         *y0_gal += dy * sig_gal_deconv;
         find_mom_1(gal_image, moments_gal, max_order_gal, *x0_gal, *y0_gal, *sig_gal);

         /* Exit if too many iterations */
#define MAX_ITER_CENTROID 400
         num_iter_centroid ++;
         if (num_iter_centroid > MAX_ITER_CENTROID) {
            find_mom_1(gal_image, moments_gal, max_order_gal, x0_gal_orig, y0_gal_orig,
               *sig_gal);
            flags &= ~ 0x00000002; /* Turns off centroid iteration flag 
                                    * since this failed.
                                    */
            status |= 0x0004; /* Report failure */
         }

     } else {
         resid_error = 0.0; /* Exit while loop */
     }
   }

   /* Now that the moments have been determined, we must compute an ellipticity
    * and its responsivity.  Note that we are neglecting the dependence of the
    * weighting function on the shear (which in principle may lead to a bias
    * if we do not have the centroid and size flags set).  The parameter a4c is
    * the circular special case of Bernstein & Jarvis's a4 parameter.  Note that
    */
   a4c = ( 0.6123724356958 * (moments_gal_deconv[4][0]+moments_gal_deconv[0][4])
         + 0.5*moments_gal_deconv[2][2] ) / moments_gal_deconv[0][0];
   *e1 = ( moments_gal_deconv[2][0] - moments_gal_deconv[0][2] )
        / ( 1.414213562373 * moments_gal_deconv[0][0] * (1.-a4c) );
   *e2 = moments_gal_deconv[1][1] / ( moments_gal_deconv[0][0] * (1.-a4c) );

   *responsivity = 1.; /* We have packaged the responsivity into a4c */

#ifdef TEST_SHAPELETS_003
   printf("%3d iterations\n",num_iter_centroid);
   printf("Mom: [0] %lf; [1] %lf,%lf; [2] %lf,%lf,%lf\n",
   moments_gal_deconv[0][0], moments_gal_deconv[1][0],
   moments_gal_deconv[0][1], moments_gal_deconv[2][0],
   moments_gal_deconv[1][1], moments_gal_deconv[0][2]);
...
#define EST_SHAPELET 5
   ...
   /* For shapelets, it's more complicated because we need to figure out
    * what order correction is desired.
    */
   if (strlen(shear_est)>8 && (!est_id)) {
      est_id = EST_SHAPELET;
      num_args = sscanf(shear_est,"SHAPELET%d,%d", &max_order_psf, &max_order_gal);
      if (num_args != 2) return 0x4000;
   }
   ...
   switch(est_id) {
      ...
      case EST_KSB:
         ...
         status = psf_corr_ksb_1(gal_image, PSF, &(gal_data->e1), &(gal_data->e2),
      ...
      case EST_REGAUSS:
         ...
         status = psf_corr_regauss(gal_image, PSF, &(gal_data->e1), &(gal_data->e2), &R,
      ...
      case EST_SHAPELET:
         ...
         status = measure_shape_ext(gal_image, PSF, max_order_psf, max_order_gal,
            &(gal_data->e1), &(gal_data->e2), &(gal_data->responsivity), &R, flags,
            &(gal_data->x0), &(gal_data->y0), &(gal_data->sigma), &(gal_data->flux),
