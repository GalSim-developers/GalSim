...
#define MAX_ASHIFT 5.0
    ...
    void convolution_matrix_1(
        double **psf_moments, int max_order_psf, int max_order_gal,
...
#define FAILED_MOMENTS (-1000.0)
    ...
    unsigned int measure_shape_ext(
        RECT_IMAGE *gal_image, RECT_IMAGE *PSF, int max_order_psf,
        ...
        *R = 1. - (*sig_psf * *sig_psf) / (*sig_gal * *sig_gal);
        ...
        while (resid_error > 1.0e-6) {
            ...
            if (flags & 0x00000002) { /* if we are iterating to find the centroid */
                ...
                dx     = moments_gal_deconv[1][0] /
                    (0.7071067811865*moments_gal_deconv[0][0] - moments_gal_deconv[2][0]);
                dy     = moments_gal_deconv[0][1] /
                    (0.7071067811865*moments_gal_deconv[0][0] - moments_gal_deconv[0][2]);
                if (dx >  BOUND_CORRECT_WEIGHT) dx =  BOUND_CORRECT_WEIGHT;
                if (dx < -BOUND_CORRECT_WEIGHT) dx = -BOUND_CORRECT_WEIGHT;
                if (dy >  BOUND_CORRECT_WEIGHT) dy =  BOUND_CORRECT_WEIGHT;
                if (dy < -BOUND_CORRECT_WEIGHT) dy = -BOUND_CORRECT_WEIGHT;
...
#define EST_SHAPELET 5
        ...
        switch(est_id) {
          ...
          case EST_SHAPELET:
               ...
               status = measure_shape_ext(
                   gal_image, PSF, max_order_psf, max_order_gal,
