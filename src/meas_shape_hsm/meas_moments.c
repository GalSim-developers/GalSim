...
int read_image(char FileName[], RECT_IMAGE *MyImage) {
...
int main(int argc, char **argv) {
  ...
  RECT_IMAGE AtlasImage, PSFImage, FiltImage, RefImage;
   int status,num_iter;
   long i,j,ii,jj;
   OBJECT_DATA GalaxyData, PSFData;
   double x00, y00, shearsig, skyvar;
   double maxval = -1.00;
   double xcentguess = 0.0,ycentguess = 0.0, normval = 0.0;
   ...
   find_ellipmom_2(&AtlasImage,&A_gal,&x00,&y00,&Mxx_gal,&Mxy_gal,&Myy_gal,&rho4_gal,1.0e-6,&num_iter);

   printf("%d %13.6lf %13.6lf %13.6lf %13.6lf %13.6lf %03d  %13.6lf %13.6lf %13.6lf\n", status, Mxx_gal,Myy_gal,Mxy_gal,(Mxx_gal-Myy_gal)/(Mxx_gal+Myy_gal),2.0*Mxy_gal/(Mxx_gal+Myy_gal),num_iter,A_gal,x00,y00);
