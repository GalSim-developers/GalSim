...
class GSObject:
    ...
    def drawShoot(self, image, N=0., ud=None, normalization="flux", noise=0., poisson_flux=True):
        """@brief Returns a tuple containing the Image of the object, with bounds set by an input 
        Image, and the total photon flux added to the image.
        ...
        before drawing in the photons.  Scale and location of the image pixels will not be altered. 

        It is important to remember that the image produced by drawShoot() represents the object
        as convolved with the square image pixel.  So do not expect an exact match, even in the 
        limit of large photon numbers N, between the outputs of draw() and drawShoot().
