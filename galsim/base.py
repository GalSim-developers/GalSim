...
are collectively known as GSObjects, and include both simple ones (like Gaussian, a 2d Gaussian
intensity profile) and compound ones (like Convolve, which represents the convolution of multiple
surface brightness profiles).  These classes also have associated methods to (a) retrieve
...
class GSObject(object):
    ...
    def drawShoot(self, image=None, dx=None, gain=1., wmult=1., normalization="flux",
        ...
        The (required) input image must have defined boundaries and pixel scale.  The photons
        generated by the drawShoot() method will be binned into the target image.  The input image
        will be cleared before drawing in the photons by default, unless the keyword add_to_image is
